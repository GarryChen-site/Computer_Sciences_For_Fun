...
void print_information(const vector<int>& vec) {
  ...
  // cout << "these two lines will not compile without supplemental material
  // (const_iterator implementation)" << endl; auto max =
  // *std::max_element(vec.begin(), vec.end()); cout << max << endl;
}
...
void still_crashes(vector<int> vec) {
  ...
  copy.at(1) = 2;
  cout << "After we change copy: " << vec.at(1)
       << " (why does changing copy impact vec?)" << endl;
