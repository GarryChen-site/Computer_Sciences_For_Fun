...
double mag(const vector<int>& v) { return sqrt(dotProduct(v, v)); }
...
int main() {
  ...
  // Compute the "angle" between feature vector of unknown and each of the three
  // authors
  cout << "Similarity - hamilton <-> unknown: "
       << angle(hamilton_vec, unknown_vec) << endl;
  cout << "Similarity - jj <-> unknown: " << angle(jj_vec, unknown_vec) << endl;
  cout << "Similarity - madison <-> unknown: "
       << angle(madison_vec, unknown_vec) << endl;
