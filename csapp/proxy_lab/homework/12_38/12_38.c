...
// function for creating server threads
...
int main(int argc, char **argv)
{
    ...
    Phtread_create(&tid, NULL, adjust_threads, NULL);
    while (1)
    {
...
void create_threads(int start, int end)
{
    ...
    for (i = start; i < end; i++)
    {
        ...
        *idx = i;
        // pass thread index in array into thread
        Pthread_create(&threads[i].tid, NULL, serve_thread, idx);
    }
}
...
void *adjust_threads(void *vargp)
{
    ...
    while (1)
    {
        // if sbuf if full, double threads
        if (sbuf_full(sp))
        {
            if (nthreads == THREAD_LIMIT)
            {
                fprintf(stderr, "thread limit reached\n");
                continue;
            }
