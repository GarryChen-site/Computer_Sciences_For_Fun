...
def quadratic(x, a, b, c):
    ...
    if lower_bound(x) <= extreme_point <= upper_bound(x):
        if a > 0:
            return interval(f(lower_bound(x)), extreme_value) if a > 0 else interval(extreme_value, f(upper_bound(x)))
        else:
            return interval(extreme_value, f(lower_bound(x))) if a > 0 else interval(f(upper_bound(x)), extreme_value)
    else:
