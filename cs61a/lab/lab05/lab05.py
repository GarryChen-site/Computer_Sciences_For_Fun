...
def add_trees(t1, t2):
    ...
    if t1 is None:
        ...
        len_diff = len(t1_branches) - len(t2_branches)
        if len_diff > 0:
            t2_branches += [None] * len_diff
        elif len_diff < 0:
            t1_branches += [None] * -len_diff
        new_branches = [add_trees(b1, b2) for b1, b2 in zip(t1_branches, t2_branches)]
